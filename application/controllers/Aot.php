...
class Aot extends CI_Controller
{
    ...
    public function manualload_monthly($date, $serialnumber, $employpin = null)
    {
        ...
        foreach ($outputtwo as $key => $processdata1) {
            ...
            foreach ($numbercal as $daycount) {
                ...
                $shiftdeducsone = $this->universal_model->selectzy(array('tol_beforestart','tolerence_afterstart','tolerence_beforeend','tolerence_afterend','shiftstarts', 'shift_end'), 'shiftdefdetails', 'day_week', $weekday, 'id_attendence_shiftdef', $id_shift_defs[0]);
				...
				$shiftmrg = $shiftdeducs['tolerence_afterstart'];
				$shiftevn = $shiftdeducs['tolerence_beforeend'];
				$shiftotmrg = $shiftdeducs['tol_beforestart'];
				$shiftotevn = $shiftdeducs['tolerence_afterend'];
                //SHIFT PER DAY
                $perdate = array();
                ...
                if (empty($m_array)) {
                    $mergie_thefrog = array(
                        ...
                        'lostt' => $this->lostt($shifto,$shiftmrg,$shiftevn, $shiftfrom, "", $valuen, $lunchmins[0], $processdata1['pin']),
                    ...
                    $perdate['lostt'] = $this->lostt($shifto,$shiftevn,$shiftmrg, $shiftfrom, $stringtime, $valuen, $lunchmins[0], $processdata1['pin']);
                    $perdate['mot15'] = $this->mot15($shiftfrom,$shiftotmrg, $stringtime, $id_shift_defs[0], $valuen, $processdata1['pin']);
                    $perdate['ot15'] = $this->ot15($shifto,$shiftotevn, $stringtime, $id_shift_defs[0], $valuen, $processdata1['pin']);
    ...
    public function lostt($shiftto,$shiftmrg, $shiftevn, $shiftfrom, $dateclocktime, $dateday, $lunchmin, $pin)
    {
    ...
    public function ot15($shiftto, $shiftotevn, $dateclocktime, $shiftid, $day, $pin)
    {
        $exemptstatus = $this->universal_model->selectz(array('exempt_ot', 'exempt_lost', 'exempt_clocking'), 'users', 'pin', $pin);
        ...
        if ($exempt_ot == 0) {
            ...
            $shiftdeducs = $this->universal_model->selectzy(array('tol_beforestart','tolerence_afterend','shiftstarts', 'shift_end'), 'shiftdefdetails', 'day_week', $weekday, 'id_attendence_shiftdef', $shiftid);
            if (empty($shiftdeducs)) {
                $logibnlogoutarray = explode('@', $dateclocktime);
                $count_login = count($logibnlogoutarray);
                $shiftotevn = date_create($shiftotevn);
                $logout = date_create($logibnlogoutarray[$count_login - 1]);
                // .......
                if ($logout > $shiftotevn) {
                    $diff = date_diff($logout, $shiftotevn);
                    $time = date('H:i:s', strtotime($diff->format("%h:%i:%s")));
                    return decimalHours($time);
                } else {
                    return '';
                ...
                $logibnlogoutarray = explode('@', $dateclocktime);
                $count_login = count($logibnlogoutarray);
                $end = $shiftdeducs[0]['tolerence_afterend'];
                $shiftotevn = date_create($end);
                $logout = date_create($logibnlogoutarray[$count_login - 1]);
    ...
    public function mot15($shiftfrom,$shiftotmrg, $dateclocktime, $shiftid, $day, $pin)
    {
        $exemptstatus = $this->universal_model->selectz(array('exempt_ot', 'exempt_lost', 'exempt_clocking'), 'users', 'pin', $pin);
        $exempt_ot = array_shift($exemptstatus)['exempt_ot'];
        //        $exempt_ot = 1;
        if ($exempt_ot == 0) {
            ...
            if (empty($shiftdeducs)) {
                $logibnlogoutarray = explode('@', $dateclocktime);
                $shiftotmrg = date_create($shiftotmrg);
                $login = date_create($logibnlogoutarray[0]);
                // .......
                if ($login < $shiftotmrg) {
                    $diff = date_diff($shiftotmrg, $login);
                    $time = date('H:i:s', strtotime($diff->format("%h:%i:%s")));
                    return decimalHours($time);
                } else {
                    return '';
                ...
                $logibnlogoutarray = explode('@', $dateclocktime);
                $start = $shiftdeducs[0]['tol_beforestart'];
                $shiftotmrg = date_create($start);
                $login = date_create($logibnlogoutarray[0]);
                if ($login < $shiftotmrg) {
                    $diff = date_diff($shiftotmrg, $login);
                    $time = date('H:i:s', strtotime($diff->format("%h:%i:%s")));
