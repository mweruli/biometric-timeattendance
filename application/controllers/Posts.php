...
class Posts extends Aot
{
    ...
    public function timetest($test_datacard, $from, $to, $summery = 1)
    {
        ...
        foreach ($test_datacard as $_cardtestgrandour) {
            ...
            $this->table->add_row($cell);
            $this->table->add_row($cellempcodenuber, $cellempcode, $cellempnames);
            $this->table->add_row('<b>Day</b>', '<b>Date</b>', '<b>Week</b>', '<b>Off</b>', '<b>Shift</b>', '<b>Login</b>', '<b>Logout</b>', '<b>Normal T</b>', '<b>Tot Hrs</b>', '<b>Lost T</b>', '<b>Mode</b>','<b>Morg OT</b>','<b>Evg OT</b>');
            $weekday = explode('~', $_cardtestgrandour['weekday']);
            $dateclock = explode('~', $_cardtestgrandour['dateclock']);
            ...
            foreach ($weekday as $key => $weekday_o) {
                ...
                if ($summery == 0) {
                    $this->table->add_row($weekday_o, $dateclock[$key], $weekyear[$key], $off[$key], $id_shift_def, $absent_state, $logout[$key], $totalnormaltime, $totalconvert, $lostt[$key], $manual[$key],$mot15[$key],$ot15[$key]);
                }
            }
            $this->table->add_row($cellspacecell, $cellspacecell, $cellspacecell, $celloff, $cellspacecell, $cellsdaysworked, $cellabsent, $celltotalhours, $celltotalworked, $cellshort, $cellspacecell,$cellmorningot,$cellot15);
            $this->table->add_row('', '', '', count($array_totaloffs), '', count($array_totaldaysworked), count($array_totaldaysabsent), $this->celltotalworked($array_totalstandhrs), $this->celltotalworked($array_totalhrs), $this->celltotalworked($arraylost),'', $monr_total,$ot15_total);
        }
        return $this->table->generate();
    }
